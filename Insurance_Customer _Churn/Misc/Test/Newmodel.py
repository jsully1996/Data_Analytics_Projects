    ...
    def policyTree(self):
        ...
        for j in com:
            if self.emp is None:
                ...
                self.emp = self.emp.join(self.cop[j])
        self.emp = self.emp.join(pd.DataFrame(self.Y, columns = ['Churn']))       
        self.emp = self.emp[self.emp['Churn'] == 1] # customer with churn values 1 are filterer here to be displayed on the table
        self.tree.delete(*self.tree.get_children())
        
        self.tree["columns"]=tuple(self.emp.columns)
        
        for i in self.emp.columns:
            self.tree.column(i,stretch = False, width=100 ) # you can change this width value in each method to properly view the spacing
            self.tree.heading(i, text=i)
        for i in range(self.examples):
            self.tree.insert("",'end',text=str(i),values=tuple(self.emp.iloc[i].values))
        ...
        for j in com:
            if self.emp is None:
                ...
                self.emp = self.emp.join(self.cop[j])
        self.emp = self.emp.join(pd.DataFrame(self.Y, columns = ['Churn']))       
        self.emp = self.emp[self.emp['Churn'] == 0] # customer with churn value 0 are filtered here
